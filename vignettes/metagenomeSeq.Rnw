...
<<createMRexperiment2>>=
OTUdata = as(taxa,"AnnotatedDataFrame")
OTUdata
@
...
the fitted coefficients represent the fold-change for our "smoker" vs. "nonsmoker" analysis. 
...
We now support experiments involving sample/feature duplicates abundance measurements are correlcted using \texttt{duplicateCorrelation}.
<<dupCor>>=
# Factor list of patient IDs
block <- pData(lungTrim)[,"patientID"]
# Fitting the model accounting for duplicate samples
res = fitZig(obj = lungTrim,mod=mod,control=settings,
             useMixedModel=TRUE,block=block)
MRtable(res,number=2)
@
Further specific details can be found in section 9.7 and beyond of the Limma user guide.
...
phylumMouse = aggTax(MRcounts(mouseData),lvl=phylum,out='matrix')
head(phylumMouse[1:5,1:5])

# Alternative, if your featureData is well maintained
aggTax(obj,lvl='phylum')
@
